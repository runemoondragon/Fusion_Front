...
const db_1 = __importDefault(require("../db")); // Or relevant path to db pool
/**
 ...
 */
async function logAdminAction(adminUserId, actionType, targetEntityType, targetEntityId, details, summary, dbClient // Can pass a PoolClient if within a transaction
) {
    const client = dbClient || db_1.default; // Use provided client or default pool
    try {
        await client.query(`INSERT INTO admin_actions_logs (admin_user_id, action_type, target_entity_type, target_entity_id, details, summary)
             VALUES ($1, $2, $3, $4, $5, $6)`, [adminUserId, actionType, targetEntityType, targetEntityId, details ? JSON.stringify(details) : null, summary]);
