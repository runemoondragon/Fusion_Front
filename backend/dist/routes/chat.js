...
async function deductCreditsAndLog(userId, cost, provider, model, tokens) {
    // Convert cost to cents
    const costCents = Math.round(cost * 100);
    // Deduct credits from user_credits
    await db_1.default.query('UPDATE user_credits SET balance_cents = balance_cents - $1 WHERE user_id = $2', [costCents, userId]);
    // Log transaction in credit_transactions
    await db_1.default.query(`INSERT INTO credit_transactions (user_id, amount_cents, method, status, description, created_at)
     VALUES ($1, $2, $3, $4, $5, NOW())`, [userId, -costCents, 'usage', 'completed', `Usage for ${provider}/${model}, ${tokens} tokens`]);
}
/**
 * @swagger
 * /api/chat:
...
router.post('/', auth_1.verifyToken, async (req, res) => {
    ...
    if (userRole === 'tester') {
        try {
            const appConfigResult = await db_1.default.query('SELECT key, value FROM app_config WHERE key = ANY($1)', [['limit_internal_api_cost_cents_tester', 'limit_neuroswitch_requests_tester']]);
            const limits = {};
      ...
      `;
            const usageResult = await db_1.default.query(usageQuery, [userId]);
            ...
            const isCurrentRequestForNeuroSwitch = ((_a = req.body.provider) === null || _a === void 0 ? void 0 : _a.toLowerCase()) === 'neuroswitch';
            if (isCurrentRequestForNeuroSwitch && neuroswitchRequestsLimit > 0) {
                if (currentMonthNeuroswitchRequests < neuroswitchRequestsLimit) {
                    ...
                    const neuroSwitchFeeCents = (await (0, costCalculator_1.getNeuroSwitchClassifierFee)()) * 100;
                    const creditResult = await db_1.default.query('SELECT balance_cents FROM user_credits WHERE user_id = $1', [userId]);
    ...
    if (userRole === 'user') {
        ...
        const appConfigResult = await db_1.default.query('SELECT value FROM app_config WHERE key = $1', ['limit_internal_api_cost_cents_tester']);
        ...
        if (internalApiCostLimitCents > 0) { // Only if limit is active
            const usageResult = await db_1.default.query(`SELECT COALESCE(SUM(CASE WHEN api_key_id IS NULL THEN cost ELSE 0 END) * 100, 0) AS current_month_internal_api_cost_cents
    ...
    if (requiresCreditPreCheck) {
        try {
            ...
            if (currentBalanceCents <= 0) {
                ...
                const isCurrentRequestForNeuroSwitch = ((_b = req.body.provider) === null || _b === void 0 ? void 0 : _b.toLowerCase()) === 'neuroswitch';
    ...
    let apiKeyIdToLog = null;
    let modelForCosting = undefined;
    const chatIdFromHeader = req.headers['x-chat-id'];
    if (!prompt) {
    ...
    if (chatIdFromHeader) { // Removed isProd condition
      ...
      ORDER BY m.timestamp ASC`, [parseInt(chatIdFromHeader, 10), userId]);
            dbHistory = historyResult.rows.map(msg => ({ role: msg.role, content: msg.content }));
        }
        catch (dbError) {
    ...
    try {
        ...
        const targetProviderForBYOAPI = (_c = payloadToNeuroSwitch.requested_provider) === null || _c === void 0 ? void 0 : _c.toLowerCase();
        if (targetProviderForBYOAPI && targetProviderForBYOAPI !== 'neuroswitch') {
            ...
            try {
                ...
                const providerDbResult = await db_1.default.query('SELECT id, name FROM providers WHERE LOWER(name) = LOWER($1)', [targetProviderForBYOAPI]);
                if (providerDbResult.rows.length > 0) {
                    ...
                    const externalKeyResult = await db_1.default.query('SELECT encrypted_api_key, is_active FROM user_external_api_keys WHERE user_id = $1 AND provider_id = $2', [userId, providerId]);
                    if (externalKeyResult.rows.length > 0 && externalKeyResult.rows[0].is_active) {
                        ...
                        try {
                            const decryptedKey = (0, crypto_1.decrypt)(encryptedKey);
            ...
            try {
                const activeKeysResult = await db_1.default.query(`SELECT p.name as provider_db_name, ueak.encrypted_api_key
           FROM user_external_api_keys ueak
           JOIN providers p ON ueak.provider_id = p.id
           WHERE ueak.user_id = $1 AND LOWER(p.name) = ANY($2::text[]) AND ueak.is_active = TRUE`, [userId, targetProvidersForNeuroSwitchRouting]);
        ...
        const neuroRes = await axios_1.default.post(neuroSwitchUrl, payloadToNeuroSwitch, { headers: neuroSwitchHeaders });
        ...
        const promptTokens = tokenUsage === null || tokenUsage === void 0 ? void 0 : tokenUsage.input_tokens;
        const completionTokens = tokenUsage === null || tokenUsage === void 0 ? void 0 : tokenUsage.output_tokens;
        const responseTime = (_d = tokenUsage === null || tokenUsage === void 0 ? void 0 : tokenUsage.runtime) !== null && _d !== void 0 ? _d : data.response_time;
        const totalTokensForLog = (_f = (_e = tokenUsage === null || tokenUsage === void 0 ? void 0 : tokenUsage.total_tokens) !== null && _e !== void 0 ? _e : data.tokens) !== null && _f !== void 0 ? _f : undefined;
        // --- Cost calculation ---
        let llmProviderCost = null;
        let neuroSwitchFeeToLog = 0; // This is the fee amount itself
        ...
        const currentRequestIsNeuroSwitchType = ((_g = req.body.provider) === null || _g === void 0 ? void 0 : _g.toLowerCase()) === 'neuroswitch';
        if (currentRequestIsNeuroSwitchType) {
            neuroSwitchFeeToLog = await (0, costCalculator_1.getNeuroSwitchClassifierFee)(); // Actual fee value
        ...
        if (totalTokensForLog !== undefined && neuroSwitchActualProvider && neuroSwitchActualProvider.toLowerCase() !== 'neuroswitch') {
            ...
            const providerDbResult = await db_1.default.query('SELECT id FROM providers WHERE LOWER(name) = $1', [providerNameFromNeuroSwitch]);
            let userApiKeyIdForActualProvider = null;
            if (providerDbResult.rows.length > 0) {
                const providerIdFromDb = providerDbResult.rows[0].id;
                const keyRes = await db_1.default.query('SELECT id FROM user_external_api_keys WHERE user_id = $1 AND provider_id = $2 AND is_active = TRUE', [userId, providerIdFromDb]);
                if (keyRes.rows.length > 0)
                    userApiKeyIdForActualProvider = keyRes.rows[0].id;
            }
            if (userApiKeyIdForActualProvider && !neuroSwitchFallbackReason) { // SUCCESSFUL BYOAPI
                ...
                modelForCosting = neuroSwitchActualModel === null || neuroSwitchActualModel === void 0 ? void 0 : neuroSwitchActualModel.toLowerCase();
                // ... (default model assignment logic as before) ...
                if (!modelForCosting && providerForCosting) {
                    if (providerForCosting === 'gemini') {
                        modelForCosting = 'gemini-1.5-flash-latest';
                    }
                    else if (providerForCosting === 'claude') {
                        modelForCosting = 'claude-3-haiku-20240307';
                    }
                    else if (providerForCosting === 'openai') {
                        modelForCosting = 'gpt-4o-mini';
                    }
                }
                if (modelForCosting) {
                    llmProviderCost = await (0, costCalculator_1.calculateLlmProviderCost)(providerForCosting, modelForCosting, promptTokens || 0, completionTokens || 0);
        ...
        if (totalCostToDeductFromCredits > 0) {
            ...
            const creditCheckResult = await db_1.default.query('SELECT balance_cents FROM user_credits WHERE user_id = $1', [userId]);
