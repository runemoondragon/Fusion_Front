...
router.get('/', async (req, res) => {
    try {
        ...
        const result = await db_1.default.query('SELECT u.id, u.email, u.display_name, u.role, u.created_at, uc.balance_cents ' +
...
router.put('/:userId/role', async (req, res) => {
    ...
    try {
        ...
        const updateResult = await client.query('UPDATE users SET role = $1, updated_at = NOW() WHERE id = $2 RETURNING id, email, role, display_name', [normalizedNewRole, targetUserId]);
        const logDetails = {
            old_role: currentRole,
            new_role: normalizedNewRole,
            target_user_id: targetUserId
        };
        await (0, adminLogger_1.logAdminAction)(adminUserId, 'USER_ROLE_UPDATED', 'USER', targetUserId.toString(), logDetails, roleChangeSummary || undefined, client);
...
router.post('/:userId/adjust-credits', async (req, res) => {
    var _a;
    const { userId: targetUserIdString } = req.params;
    ...
    try {
        ...
        const currentBalanceCents = ((_a = creditResult.rows[0]) === null || _a === void 0 ? void 0 : _a.balance_cents) || 0;
        ...
        const updateCreditResult = await client.query(`INSERT INTO user_credits (user_id, balance_cents, updated_at)
       VALUES ($1, $2, NOW())
       ON CONFLICT (user_id)
       DO UPDATE SET balance_cents = user_credits.balance_cents + $3, updated_at = NOW()
       RETURNING balance_cents AS new_balance_cents`, [targetUserId, amount_cents, amount_cents] // For INSERT, it's amount_cents directly. For UPDATE, it's current + amount_cents.
        ...
        const finalNewBalanceResult = await client.query(`INSERT INTO user_credits (user_id, balance_cents, updated_at)
         VALUES ($1, $2, NOW())
         ON CONFLICT (user_id)
         DO UPDATE SET balance_cents = user_credits.balance_cents + $3, updated_at = NOW()
         RETURNING balance_cents`, [targetUserId, projectedNewBalance, amount_cents] // If inserting new, use projected. If updating, add amount.
        ...
        const finalUpsertResult = await client.query(`INSERT INTO user_credits (user_id, balance_cents, updated_at)
         VALUES ($1, $2, NOW())
         ON CONFLICT (user_id)
         DO UPDATE SET balance_cents = $2, updated_at = NOW()
         RETURNING balance_cents`, [targetUserId, projectedNewBalance]);
        ...
        await client.query(`INSERT INTO credit_transactions (user_id, amount_cents, method, status, description, created_at)
       VALUES ($1, $2, 'admin_adjustment', 'completed', $3, NOW())`, [targetUserId, amount_cents, transactionDescription]);
        // 6. Log admin action
        const logDetails = {
        ...
        await (0, adminLogger_1.logAdminAction)(adminUserId, 'USER_CREDIT_ADJUSTMENT', 'USER', targetUserId.toString(), logDetails, reason, client);
        await client.query('COMMIT');
        res.status(200).json({
