...
const crypto_1 = __importDefault(require("crypto")); // <-- Import crypto for HMAC
const auth_1 = require("../middleware/auth");
const db_1 = __importDefault(require("../db")); // <-- Import pool for database operations
...
router.post('/create-invoice', auth_1.verifyToken, async (req, res) => {
...
router.post('/btcpay', async (req, res) => {
    ...
    try {
        const hmac = crypto_1.default.createHmac('sha256', BTCPAY_WEBHOOK_SECRET);
        hmac.update(req.rawBody); // Use req.rawBody (Buffer) for HMAC
        const digest = 'sha256=' + hmac.digest('hex');
        if (!crypto_1.default.timingSafeEqual(Buffer.from(digest), Buffer.from(sig))) {
        ...
        if (event.type === 'InvoiceSettled') {
            ...
            try {
                ...
                const existingTx = await client.query('SELECT id FROM credit_transactions WHERE provider_transaction_id = $1 AND method = $2 AND status = $3', [invoiceId, 'btcpay', 'completed']);
