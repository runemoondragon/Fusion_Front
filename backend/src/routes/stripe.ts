...
import pool from '../db'; // Assuming your DB pool is exported from here
...
router.post('/create-session', verifyToken, async (req: Request, res: Response) => {
...
router.get('/billing-portal', verifyToken, async (req: Request, res: Response) => {
  const authenticatedUser = req.user as AuthUser;
  const userId = authenticatedUser?.id;
...
router.post('/stripe', async (req: Request, res: Response) => { // Removed express.raw(), path becomes /stripe
  ...
  switch (event.type) {
    case 'checkout.session.completed':
      ...
      if (session.payment_status === 'paid') {
        ...
        const amountCents = session.metadata?.credit_amount_cents ? parseInt(session.metadata.credit_amount_cents, 10) : null;
        ...
        try {
          ...
          const existingTx = await dbClient.query(
            'SELECT id FROM credit_transactions WHERE provider_transaction_id = $1 AND method = $2',
            [stripeTransactionId, 'stripe']
          );
...
router.post('/create-setup-intent', verifyToken, async (req: Request, res: Response) => {
...
router.post('/save-payment-method-details', verifyToken, async (req: Request, res: Response) => {
    const user = req.user as AuthUser;
    if (!user || !user.id) {
        return res.status(403).json({ error: 'User not authenticated' });
    }
    ...
    try {
        ...
        await pool.query(query, [user.id, paymentMethodId, last4, cardBrand]);
...
router.get('/auto-topup-details', verifyToken, async (req: Request, res: Response) => {
    const user = req.user as AuthUser;
    if (!user || !user.id) {
        return res.status(403).json({ error: 'User not authenticated' });
    }
    ...
    try {
        const result = await pool.query(
            'SELECT stripe_payment_method_id, is_enabled, threshold_cents, topup_amount_cents, last4, card_brand FROM user_auto_topup_settings WHERE user_id = $1',
            [user.id]
        );
...
router.post('/update-auto-topup-settings', verifyToken, async (req: Request, res: Response) => {
  const user = req.user as AuthUser;
  if (!user || !user.id) {
    return res.status(403).json({ error: 'User not authenticated' });
  }
  ...
  try {
    const settingsExistResult = await pool.query(
      'SELECT stripe_payment_method_id FROM user_auto_topup_settings WHERE user_id = $1',
      [user.id]
    );
...
router.post('/remove-payment-method', verifyToken, async (req: Request, res: Response) => {
  const user = req.user as AuthUser;
  if (!user || !user.id) {
    return res.status(403).json({ error: 'User not authenticated' });
  }
